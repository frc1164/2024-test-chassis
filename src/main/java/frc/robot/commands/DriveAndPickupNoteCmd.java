...
public class DriveAndPickupNoteCmd extends Command {
  ...
  private double LimeLightHeight = 0.9806;                // The LimeLight distance off the floor in meters, used to estimate distance to a NOTE.
  ...
  public DriveAndPickupNoteCmd(SwerveSubsystem swerveSubsystem) {
    ...
    this.limelighNetworkTable = NetworkTableInstance.getDefault().getTable(VisionConstants.kShootLimelightNetworkTableName);
    ...
    if(detectNotes == true) {
      ...
      if(ta >= noteSizeThreshold && Math.abs(tx) <= 5) {
        ...
        if((lateralDistanceToNote == 0 || ty >= minimumNoteAngle) && !moveToNote) {
          distanceFromNote = Math.tan((90 - Math.abs(ty) - 17) * Math.PI/180) * LimeLightHeight;
          moveToNote = true;
        }
